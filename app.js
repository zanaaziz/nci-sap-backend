...
app.get('/translations', authenticateJWT, (req, res) => {
  pool.query('SELECT nodeId, language, translation FROM translations', (err, results) => {
    if (err) return res.status(500).json({ error: 'Database error' });
...
app.post('/translations', authenticateJWT, (req, res) => {
  ...
  const values = translations.flatMap(t => [t.nodeId, t.language, t.translation]);
  const placeholders = translations.map((_, i) => `($${i * 3 + 1}, $${i * 3 + 2}, $${i * 3 + 3})`).join(', ');
  const query = `
    INSERT INTO translations (nodeId, language, translation)
    VALUES ${placeholders}
    ON CONFLICT (nodeId, language) DO UPDATE SET translation = EXCLUDED.translation
  `;
